...
set signUpPw(v: string) {
  ...
  loginProcess = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:4200/api/login", { id: this.loginId, password: hashLoginPw });
  ...
  async signUpProcess (): Promise<void>{
        ...
        try {
          ...
          await axios.post("http://localhost:4200/api/signUp", { id: this.signUpId, password: hashSignUpPw });
        }
        catch (e) {
          if (axios.isAxiosError(e) && e.response) {
            const { data } = e.response;
